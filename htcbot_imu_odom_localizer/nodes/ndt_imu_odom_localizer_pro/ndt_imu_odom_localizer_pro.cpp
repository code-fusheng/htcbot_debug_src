...
static sensor_msgs::Imu imu_odom;
...
void NdtLocalizerPro::init() {
    ...
    //  新增处理IMU数据并且预测车辆位置姿态函数 imu_callback
     sub_imu_data_ = nh.subscribe("/imu_raw", 10, &NdtLocalizerPro::imu_callback, this);
    // pub
    ...
    pub_ndt_last_poses_linear= nh.advertise<geometry_msgs::PoseStamped>("/predict_pose_imu_odom", 10);   
...
//仅用 imu
  // guess_pose_imu.x   =   offset_imu_x;
  // guess_pose_imu.y   = offset_imu_y;
  // guess_pose_imu.z   = offset_imu_z;
  // guess_pose_imu.roll = offset_imu_roll; 
  // guess_pose_imu.pitch = offset_imu_pitch;
  // guess_pose_imu.yaw  =  offset_imu_yaw;



//仅用ndt

  // guess_pose_imu.x   =  previous_pose_.x;
  // guess_pose_imu.y   =previous_pose_.y;
  // guess_pose_imu.z   = previous_pose_.z ;
  // guess_pose_imu.roll = previous_pose_.roll ;
  // guess_pose_imu.pitch = previous_pose_.pitch ;
  // guess_pose_imu.yaw  = previous_pose_.yaw  ;
...

{
 ...
 //坐标变换  由  前y右x  变换为  前x  左y
  imu_roll  =  input->orientation.y;
  imu_pitch =  -1.0*input->orientation.x ;
  imu_yaw=     input->orientation.z;
  imu.header = input->header;
  imu.linear_acceleration.x = input->linear_acceleration.y ;
  imu.linear_acceleration.y =  input->linear_acceleration.x*-1.0;
  imu.linear_acceleration.z = input->linear_acceleration.z;


  imu_odom.angular_velocity.x = input->angular_velocity.y ;
  imu_odom.angular_velocity.y =  input->angular_velocity.x*-1.0;
  imu_odom.angular_velocity.z = input->angular_velocity.z;
...
//将弧度值 限制在 正负 3.14 之间
  ...
  imu.orientation.x=diff_imu_roll ;

  imu.orientation.y= diff_imu_pitch;
  imu.orientation.z= diff_imu_yaw ;
...
void NdtLocalizerPro::callbackVehicleStatus(const can_msgs::vehicle_status::ConstPtr &msg) 
{
    ...
    static ros::Time previous_time_odom = current_time;
    // 计算前后两次接收到消息的微小时间差
    diff_time_odom=(current_time_odom - previous_time_odom).toSec();
    //  从底盘得到的 车速 与 转向
    ...
    //  利用imu的 角速度 得到 姿态角
    double diff_imu_roll = imu.angular_velocity.x * diff_time_odom;
    double diff_imu_pitch = imu.angular_velocity.y * diff_time_odom;
    double diff_imu_yaw = imu.angular_velocity.z *  diff_time_odom;
      //  利用车速  得到 位移
    double diff_odom_distance = current_vehicle_speed_*diff_time_odom;
   ...
   // 仅基于  航向角  得到  x  y  
   //  offset_odom_x += diff_odom_distance * cos(current_pose_odom.yaw);
   //  offset_odom_y += diff_odom_distance * sin(current_pose_odom.yaw);
   //  offset_odom_z = 0;  // 这里的 offset_odom_z 也可以直接忽略或保持不变


   //   基于  航向角 与 俯仰角 得到  x  y z 
   offset_odom_x = diff_odom_distance *cos(-current_pose_odom.pitch)* cos(current_pose_odom.yaw);
   offset_odom_y = diff_odom_distance * cos(-current_pose_odom.pitch) * sin(current_pose_odom.yaw);
   offset_odom_z = diff_odom_distance * sin(current_pose_odom.pitch);
  

     //  累积 x  y z  分量位移得到全局位置
   current_pose_odom.x +=  offset_odom_x ;
   current_pose_odom.y +=  offset_odom_y;
   current_pose_odom.z +=  offset_odom_z;




   previous_time_odom = current_time_odom;



  //发布  imu_odom 位置姿态
     
  geometry_msgs::PoseStamped  predict_pose_for_imu_odom;



    predict_pose_for_imu_odom.header.frame_id = "map";
    // current_pose_msg.header.stamp = current_time;
    predict_pose_for_imu_odom.pose.position.x = current_pose_odom.x ;
    predict_pose_for_imu_odom.pose.position.y = current_pose_odom.y;
    predict_pose_for_imu_odom.pose.position.z = current_pose_odom.z;

    double cy = cos(current_pose_odom.yaw * 0.5);  
    double sy = sin(current_pose_odom.yaw* 0.5);  
    double cp = cos( current_pose_odom.pitch* 0.5);  
    double sp = sin(current_pose_odom.pitch* 0.5);  
    double cr = cos( current_pose_odom.roll* 0.5);  
    double sr = sin( current_pose_odom.roll* 0.5);  
   ...
   predict_pose_for_imu_odom.pose.orientation.x =q.x();
   predict_pose_for_imu_odom.pose.orientation.y = q.y();
   predict_pose_for_imu_odom.pose.orientation.z = q.z();
   predict_pose_for_imu_odom.pose.orientation.w = q.w();


   pub_ndt_last_poses_linear.publish(predict_pose_for_imu_odom);
}
...
void NdtLocalizerPro::callbackPoints(const sensor_msgs::PointCloud2::ConstPtr& input) {
   ...
   //  使用  轮式里程计定位
    predict_pose_for_ndt = current_pose_odom;
