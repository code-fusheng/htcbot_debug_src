...
private:
    ...
    sensor_msgs::Imu imu_data; // 存储IMU数据的变量
    ros::NodeHandle nh;
    ...
    void callbackImuData(const sensor_msgs::Imu::ConstPtr& input);
    ...
    ros::Publisher  pub_imu_current_pose_ ;       // imu当前位姿话题
    void imu_calc(ros::Time current_time,const sensor_msgs::Imu::ConstPtr &input);
    Pose   guess_pose_imu;   // imu预测当前位姿

    Pose   current_pose_odom;  //里程计预测当前位姿
    ros::Time current_time ;
    ros::Time previous_times ;
    double diff_time ;

    ros::Time current_time_odom;   //当前帧里程计时间

    ros::Time previous_time_odom;    //上一帧里程计时间

    double diff_time_odom ;      // 两帧里程计时间

    double offset_odom_x, offset_odom_y, offset_odom_z, offset_odom_roll, offset_odom_pitch, offset_odom_yaw;

    double   R=0.5;
    double    angular_velocity_odom=0.0;
