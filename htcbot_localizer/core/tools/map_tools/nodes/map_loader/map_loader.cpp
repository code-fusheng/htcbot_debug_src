...
void MapLoader::callbackMapPathConf(const htcbot_msgs::MapPathConf::ConstPtr &msg) {
	...
	// dir_static_map_ = "/home/lzl/static_map" ;
	// dir_dynamic_map_ = "/home/lzl/dynamic_map";

	    ROS_INFO("[map_loader] ==>dir_static_map_: %s,  dir_dynamic_map_: %s", dir_static_map_ , dir_dynamic_map_);
...
void MapLoader::callbackInitialPose(const geometry_msgs::PoseWithCovarianceStamped &input) {
    ...
    if (dir_dynamic_map_ == "") {
	 ...
	 std::cout << "----------------------[map_loader] Received /initialpose, but dynamic map dir is empty------------------------"  <<std::endl;
...
void MapLoader::convertPointCloudToOccupancyGrid(const pcl::PointCloud<pcl::PointXYZI>::ConstPtr &in_cloud, nav_msgs::OccupancyGrid &out_occ_msg) {
	...
	for(int iter = 0; iter < in_cloud->points.size(); iter++)
	{
		...
		if(i < 0 || i >= out_occ_msg.info.width) continue;
		int j = int((in_cloud->points[iter].y - y_min) / map_resolution_);
		if(j < 0 || j >= out_occ_msg.info.height - 1) continue;
...
bool MapLoader::publishDynamicPcdMap() {
	bool load_flag = true;
	 ...
	 std::cout << "-------------------------------current_dynamic_pcd_map_msg_.width ------------------" <<current_dynamic_pcd_map_msg_.width <<std::endl;
