...
void NdtLocalizerPro::init() {
    ...
    // sub_odm_data_= nh.subscribe("/vehicle_status", 10, &NdtLocalizerPro::odomcallback, this);
...

{
  ...
  // std::cout <<"=====a_num  <<"  "<< a_max======="<<a_num  <<"  "<< a_max<<std::endl;
  if (a_num >= a_max)
  {
    a_num -= 2.0 * a_max;
  }
...
double diff_imu_roll2 = imu.orientation.x;
double diff_imu_pitch2 = imu.orientation.y;
double diff_imu_yaw2 = imu.orientation.z;
...
//   gx = std::sin(imu_pitch);  // 计算重力分量
//   gy =  std::sin(imu_roll);  // 计算重力分量
//   gz = std::cos(imu_pitch) * std::cos(imu_roll);  // 计算重力分量
...
//  imu.linear_acceleration.x =  imu.linear_acceleration.x -gx;
//  imu.linear_acceleration.x =   imu.linear_acceleration.x *-1.0;

//  imu.linear_acceleration.y =   imu.linear_acceleration.y -gy;
//  imu.linear_acceleration.z =  imu.linear_acceleration.z -gz;
