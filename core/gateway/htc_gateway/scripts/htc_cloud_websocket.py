...
class HtcCloudWebSocketNode:
    ...
    def init(self):
        ...
        self.vin = rospy.get_param("htcbot/ws/vin", "HTCBOT_A000T")
        self.sid = rospy.get_param("htcbot/ws/sid", "1000T")
        self.websocket_ip = rospy.get_param("htcbot/ws/ip", "118.190.156.22")
        self.websocket_uri = f"ws://{self.websocket_ip}:10240/robot/push/websocket?sid={self.sid}&vin={self.vin}"
    ...
    def image_callback(self, msg, camera_index):
        ...
        try:
            ...
            return
        # _, jpeg_image = cv2.imencode('.jpg', cv_image, [int(cv2.IMWRITE_JPEG_QUALITY), 20])
