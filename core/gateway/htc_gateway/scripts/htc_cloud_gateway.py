...
LastEditTime: 2024-05-29 17:34:53
Description: 
'''
...
class HtcCloudGatewayNode:
    ...
    def __init__(self):
        ...
        self.system_status = StatusHtcbotSystem()
        self.last_task_report_time = rospy.Time().now()
    ...
    def init(self):
        self.client_id = rospy.get_param("htcbot/mqtt/cid", "HTCBOT_A000*")
        self.broker = rospy.get_param("htcbot/mqtt/broker", "118.190.156.22")
        self.port   = rospy.get_param("htcbot/mqtt/port", 1883)
        self.username = rospy.get_param("htcbot/mqtt/username", "htcbot")
        self.password = rospy.get_param("htcbot/mqtt/password", "htcbot123456")
        # self.vin = rospy.get_param("/htc_cloud_gateway_node/htcbot_vin", "HTCBOT_A000*")
        self.vin = rospy.get_param("htcbot/uuid", "HTCBOT_*")
        self.vtype = rospy.get_param("htcbot/type", "none")
        ...
        self.sub_module_status_array = rospy.Subscriber("/htcbot/module_status_array", StatusHtcbotModuleArray, self.callback_module_status_array)
        self.sub_sensor_status_array = rospy.Subscriber("/htcbot/sensor_status_array", StatusHtcbotSensorArray, self.callback_sensor_status_array)
        self.sub_system_status = rospy.Subscriber("/htcbot/system_status", StatusHtcbotSystem, self.callback_system_status)
    ...
    def init_status(self):
        ...
        self.system_status.reason = "Initing"
    ...
    def callback_current_pose(self, msg):
        ...
        if self.task_start_pose and self.task_start_time is not None:
            # rospy.loginfo("=======> task time {}".format((rospy.Time().now() - self.task_start_time).to_sec()))
            ...
            if math.fabs(self.current_pose.pose.position.x - self.task_start_pose.pose.position.x) < 1.0 and math.fabs(self.current_pose.pose.position.y - self.task_start_pose.pose.position.y) < 1.0 :
                ...
                rospy.loginfo("[htc_cloud_gateway] ===> auto report stop !!!")
    ...
    def callback_module_status_array(self, msg):
        self.module_status_array.status_array = [
            {
                "module_type": item.module_type,
                "module_status": item.module_status,
                "reason": item.reason
            } for item in msg.status_array
        ]

    def callback_sensor_status_array(self, msg):
    ...
    def send_heartbreat(self):
        ...
        pub_msg = {
                    ...
                    "vehicle_status": {
                        ...
                        "steer": self.vehicle_status.cur_steer,
                        "shift_level": self.vehicle_status.shift_level,
                        "brake_level": self.vehicle_status.brake_level
                    },
                    "battery_capacity": self.battery_status.capacity,
                    "module_status": [{ "status": item["module_status"],
                                        "type": item["module_type"],
                                        "reason": item["reason"]
                                    } for item in self.module_status_array.status_array],
                    "sensor_status": [{ "status": item["sensor_status"],
                                        "type": item["sensor_type"],
                                        "reason": item["reason"]
                                    } for item in self.sensor_status_array.status_array],
                    "system_status": {
