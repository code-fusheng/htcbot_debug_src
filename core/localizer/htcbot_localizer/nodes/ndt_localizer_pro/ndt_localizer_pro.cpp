 ...
 * @LastEditTime: 2024-06-05 11:38:55
 * @Description:
 */
...
namespace NdtLocalizerProNS
{
    ...
    void NdtLocalizerPro::init()
    {
        ...
        nh_private_.param<std::string>("/ndt_localizer_pro_node/gnss_topic", gnss_topic_, "gnss/utm_pose");
        nh_private_.param<std::string>("/ndt_localizer_pro_node/localizer_mode", localizer_mode_, "LIDAR");
        ...
        // sub
        if (localizer_mode_ == "GNSS")
        {
            sub_gnss_ = nh.subscribe(gnss_topic_, queue_size_, &NdtLocalizerPro::callbackGnssPose, this);
        }
        else
        {
    ...
    void NdtLocalizerPro::callbackGnssPose(const geometry_msgs::PoseStamped::ConstPtr &input)
    {
        // 是否初始化角度
        ...
        tf::Matrix3x3 m(q);
        // 强制Z轴0
        current_pose_.x = input->pose.position.x;
        current_pose_.y = input->pose.position.y;
        current_pose_.z = input->pose.position.z;
        m.getRPY(current_pose_.roll, current_pose_.pitch, current_pose_.yaw);
    ...
    void NdtLocalizerPro::callbackPoints(const sensor_msgs::PointCloud2::ConstPtr &input)
    {
        ...
        diff_time_ = (current_scan_time_ - previous_scan_time_).toSec();
        ...
        predict_pose_msg.header.stamp = ros::Time::now();
        ...
        ndt_pose_msg.header.stamp = ros::Time::now();
