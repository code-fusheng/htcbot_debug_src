...
namespace PurePursuitNS
{
    ...
    void PurePursuit::publishControlCommandStamped(const bool &can_get_curvature, const double &curvature)
    {
        ...
        if (current_pose_closet_index >= end_waypoint_index - 5)
        { // TODO:: 增加考虑刹车距离, end_waytpoint_index应减去 (刹车距离/点密度)
            // ecu_ctl.shift = ecu_ctl.SHIFT_D;
    ...
    double PurePursuit::computeLookaheadDistance() const
    {
        ...
        return ld < (minimum_lookahead_distance_) ? minimum_lookahead_distance_ : (ld > maximum_lookahead_distance) ? maximum_lookahead_distance
                                                                                                                    : ld;
    }
