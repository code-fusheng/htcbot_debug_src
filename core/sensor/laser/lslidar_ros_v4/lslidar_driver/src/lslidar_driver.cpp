...
namespace lslidar_driver
{
    LslidarDriver::LslidarDriver(ros::NodeHandle &node, ros::NodeHandle &private_nh) : nh(node),
                                                                                       ...
                                                                                       lidar_number_(1),
                                                                                       sweep_data(
                                                                                           new lslidar_msgs_v4::LslidarScan()),
                                                                                       sweep_data_bak(
                                                                                           new lslidar_msgs_v4::LslidarScan()),
    ...
    void LslidarDriver::difopPoll()
    {
        ...
        lslidar_msgs_v4::LslidarPacketPtr difop_packet_ptr(new lslidar_msgs_v4::LslidarPacket);
        ...
        while (ros::ok())
        {
            ...
            if (rc == 0)
            {
                ...
                {
                    int total_working_time = (difop_packet_ptr->data[105] << 24) + (difop_packet_ptr->data[106] << 16) + (difop_packet_ptr->data[107] << 8) + difop_packet_ptr->data[108];
                    int less_minus40_degree_working_time = (difop_packet_ptr->data[112] << 16) + (difop_packet_ptr->data[113] << 8) +
                                                           difop_packet_ptr->data[114];
                    int minus40_to_minus10_working_time = (difop_packet_ptr->data[115] << 16) + (difop_packet_ptr->data[116] << 8) +
                                                          difop_packet_ptr->data[117];
                    int minus10_to_30_working_time = (difop_packet_ptr->data[118] << 16) + (difop_packet_ptr->data[119] << 8) + difop_packet_ptr->data[120];
                    int positive30_to_70_working_time = (difop_packet_ptr->data[121] << 16) + (difop_packet_ptr->data[122] << 8) +
                                                        difop_packet_ptr->data[123];
                    int positive70_to_100_working_time = (difop_packet_ptr->data[124] << 16) + (difop_packet_ptr->data[125] << 8) +
    ...
    {
        if (sweep_data_bak->points.size() < 65)
            return;
    ...
    // bool LslidarDriver::powerOn(lslidar_msgs_v4::lslidar_control::Request &req,
    //                             lslidar_msgs_v4::lslidar_control::Response &res) {
    //     ROS_WARN("--------------------------");
    //     // sleep(1);
    //     lslidar_msgs_v4::LslidarPacketPtr packet0(new lslidar_msgs_v4::LslidarPacket);
    //     packet0->data[0] = 0x00;
    //     packet0->data[1] = 0x00;
    //     int rc_msop = -1;
    ...
    bool LslidarDriver::timeService(lslidar_msgs_v4::time_service::Request &req,
                                    lslidar_msgs_v4::time_service::Response &res)
    {
    ...
    bool LslidarDriver::motorControl(lslidar_msgs_v4::motor_control::Request &req,
                                     lslidar_msgs_v4::motor_control::Response &res)
    {
        if (!is_get_difop_)
        {
            res.result = 0;
    ...
    bool LslidarDriver::motorSpeed(lslidar_msgs_v4::motor_speed::Request &req,
                                   lslidar_msgs_v4::motor_speed::Response &res)
    {
        if (!is_get_difop_)
        {
            res.result = 0;
    ...
    bool LslidarDriver::setDataPort(lslidar_msgs_v4::data_port::Request &req,
                                    lslidar_msgs_v4::data_port::Response &res)
    {
        if (!is_get_difop_)
        {
            res.result = 0;
    ...
    bool LslidarDriver::setDevPort(lslidar_msgs_v4::dev_port::Request &req,
                                   lslidar_msgs_v4::dev_port::Response &res)
    {
        if (!is_get_difop_)
        {
            res.result = 0;
    ...
    bool LslidarDriver::setDataIp(lslidar_msgs_v4::data_ip::Request &req,
                                  lslidar_msgs_v4::data_ip::Response &res)
    {
        std::regex ipv4(
            "\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\b");
    ...
    bool LslidarDriver::setDestinationIp(lslidar_msgs_v4::destination_ip::Request &req,
                                         lslidar_msgs_v4::destination_ip::Response &res)
    {
        std::regex ipv4(
            "\\b(?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\b");
    ...
    bool LslidarDriver::poll()
    {
        ...
        lslidar_msgs_v4::LslidarPacketPtr packet(new lslidar_msgs_v4::LslidarPacket());
        ...
        if (!start_process_msop_)
            return false;
        // packet timestamp
        lslidar_msgs_v4::LslidarPacket pkt = *packet;
        if (!is_get_scan_altitude_ && pkt.data[1211] == 0x46)
        { // old C32w
        ...
        for (size_t fir_idx = start_fir_idx; fir_idx < end_fir_idx; ++fir_idx)
        {
            ...
            sweep_data->points.push_back(lslidar_msgs_v4::LslidarPoint());
            lslidar_msgs_v4::LslidarPoint &new_point = sweep_data->points[sweep_data->points.size() - 1];
        ...
        if (end_fir_idx != SCANS_PER_PACKET)
        {
            ...
            std::thread pointcloud_pub_thread([this]
                                              { publishPointcloud(); });
            pointcloud_pub_thread.detach();

            //            if (publish_scan) publishScan();
            sweep_data = lslidar_msgs_v4::LslidarScanPtr(new lslidar_msgs_v4::LslidarScan());
